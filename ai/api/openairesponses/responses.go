// Package openairesponses implements streaming against OpenAI's /v1/responses
// API (SSE, stream=true), used by OpenAI's newer "Responses" models (e.g.
// gpt-5*). Pragmatic Go port of pi-ai's openai-responses.ts /
// openai-responses-shared.ts (stream path only; grammar tools, deferred
// tools, and tool-search are not ported).
...
const (
	...
	minOutputTokens  = 16 // OpenAI Responses rejects max_output_tokens below 16.
)

func init() {
	ai.RegisterAPI(ai.APIOpenAIResponses, Stream)
}
...
func run(out *ai.AssistantMessageEventStream, model ai.Model, ctxData ai.Context, opts *ai.SimpleStreamOptions) {
	...
	httpReq, err := http.NewRequestWithContext(httpCtx, http.MethodPost, baseURL+"/responses", bytes.NewReader(body))
	if err != nil {
...
func convertMessages(model ai.Model, ctxData ai.Context) []any {
	...
	if ctxData.SystemPrompt != "" {
		...
		out = append(out, roleMessageItem{Role: role, Content: ctxData.SystemPrompt})
	}
